...
<head>
    ...
    <title>{{ __('page.adminLoginTitle') }} | QuickClean</title>
    ...
    <div class="min-h-screen flex flex-col items-center justify-center px-6 py-10" style="background-color: #040319">
        ...
        <div class="w-full max-w-md bg-white rounded-[2.5rem] shadow-2xl p-10 border border-gray-100">
            <div class="mb-10 text-center">
                <h1 class="mt-4 text-3xl font-black text-gray-900 tracking-tight">{{ __('page.adminLoginTitle') }}</h1>
            </div>
            @if ($errors->any())
            ...
            <form action="{{ url('/admin/login') }}" method="POST" class="space-y-6">
                @csrf
                <div>
                    <label class="text-xs font-bold text-gray-400 uppercase tracking-widest ml-1">{{ __('page.emailLabel') }}</label>
