...
@section('title', __('page.editServiceProvider') . ' | QuickClean')

@section('content')
<div style="max-width: 600px; margin: 0 auto;">
    <h2 style="margin-bottom: 20px;">{{ __('page.editServiceProvider') }}: {{ $provider->name }}</h2>
    ...
    <div class="admin-section" style="background: #fff; padding: 25px; border-radius: 12px; border: 1px solid #e2e8f0;">
        <form action="{{ route('admin.providers.update', $provider->id) }}" method="POST">
            ...
            <div style="display: flex; gap: 10px;">
                <button type="submit" style="flex: 1; background: #2563eb; color: white; border: none; padding: 12px; border-radius: 8px; font-weight: 600; cursor: pointer;">{{ __('page.update') }}</button>
